...
func GetUpdatedCharts(current, previous string) (map[string]Chart, error) {
	currentCharts, err := getChartsFromVersion(current)
	if err != nil {
		return nil, err
	}

	previousCharts, err := getChartsFromVersion(previous)
	if err != nil {
		return nil, err
	}
